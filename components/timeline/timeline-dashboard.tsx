...
export function TimelineDashboard() {
  ...
  function saveActivity(draft: ActivityDraft) { if (editing) { updateActivity(editing.id, draft); showToast("Activity updated"); } else { addActivity(draft); showToast("Activity added to Timeline"); } closeForm(); }
  function editActivity(activity: Activity) { setEditing(activity); setFormKind(activity.kind); }
  function confirmDelete() { if (!deleting) return; removeActivity(deleting.id); showToast("Activity removed"); setDeleting(undefined); }
  return <div className="timeline-dashboard"><TimelineHeader dateKey={dateKey} onDateChange={setDateKey} /><TodaySummaryCard activities={dayActivities} /><ActivityFilterChips active={filter} onChange={setFilter} />{visibleActivities.length ? <ActivityTimeline activities={visibleActivities} onEdit={editActivity} onDelete={setDeleting} /> : <section className="timeline-empty"><p>No activities logged yet.</p><button type="button" className="button button--primary" onClick={() => setQuickAddOpen(true)}>Add activity</button></section>}<FloatingAddButton onClick={() => setQuickAddOpen(true)} /><QuickAddBottomSheet open={quickAddOpen} onClose={() => setQuickAddOpen(false)} onSelect={openNewForm} /><ActivityFormSheet open={Boolean(formKind)} kind={formKind} dateKey={dateKey} activity={editing} onClose={closeForm} onSave={saveActivity} /><ConfirmDeleteDialog open={Boolean(deleting)} title={deleting?.title ?? "activity"} onCancel={() => setDeleting(undefined)} onConfirm={confirmDelete} /><Toast message={toast} /></div>;
}
