...
export function QuickAddBottomSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const onKeyDown = (event: KeyboardEvent) => { if (event.key === "Escape") { setSelected(null); onClose(); } };
    window.addEventListener("keydown", onKeyDown);
    return () => { document.body.style.overflow = originalOverflow; window.removeEventListener("keydown", onKeyDown); };
  }, [open, onClose]);

  function closeSheet() { setSelected(null); onClose(); }
  ...
  return (
    <div className={`bottom-sheet-layer${open ? " bottom-sheet-layer--open" : ""}`} aria-hidden={!open} inert={!open}>
      ...
      <section className="quick-add-sheet" role="dialog" aria-modal="true" aria-label="Quick add activity" onPointerDown={handlePointerDown} onPointerUp={handlePointerUp}>
        ...
        {selected ? <p className="quick-add-sheet__confirmation" role="status">{selected} selected</p> : null}
        <div className="quick-add-sheet__grid">
          {quickAddOptions.map((option) => { const Icon = option.icon; return <button key={option.label} type="button" onClick={() => setSelected(option.label)}><Icon size={21} aria-hidden="true" /><span>{option.label}</span></button>; })}
        </div>
      </section>
    </div>
  );
