...
export function NextMilestoneCard() {
  ...
  if (!next) return null;
  return <section className="next-milestone-card"><div><Sparkles size={18} aria-hidden="true" /><p>Next milestone</p></div><h2>{profile.preferredName} may soon begin {next.title.toLowerCase()}.</h2><span>General developmental guidance</span><Link href="/milestones">Explore milestones</Link></section>;
}
