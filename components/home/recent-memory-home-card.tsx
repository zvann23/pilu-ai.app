...
export function RecentMemoryHomeCard() {
  ...
  return <section className="recent-memory-home-card" aria-labelledby="recent-memory-title"><div className="recent-memory-home-card__image">{memory.imagePreview ? <img src={memory.imagePreview} alt={memory.imageAlt || memory.title} /> : <Images size={25} aria-hidden="true" />}</div><div><p>Recent memory</p><h2 id="recent-memory-title">{memory.title}</h2><span>{formatMemoryDate(memory.date)}</span>{memory.favorite ? <small><Heart size={13} fill="currentColor" aria-hidden="true" />Kept as a favorite</small> : null}<Link href="/memory-book">Open Memory Book</Link></div></section>;
}
